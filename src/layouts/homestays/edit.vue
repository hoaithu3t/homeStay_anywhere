<template>
    <TheHeader />
    <a-card title="Update Homestay" class="container mt-5">
...
export default {
    ...
    setup() {
        ...
        const getHomestay = () => {
            axios.get(`http://127.0.0.1:8000/api/homestays/${route.params.id}`)
                .then((response) => {
                    console.log(response);
                    formState.name = response.data.data.name;
                    formState.location_id = response.data.data.location_id;
                    formState.address = response.data.data.address;
                    formState.avatar = response.data.data.avatar;
                    formState.images = JSON.parse(response.data.data.images).map((image, index) => {
                        return {
                            uid: index + 1,
                            name: image.substring(7),
                            status: 'done',
                            url: 'http://localhost:8000/storage/' + image,
                        }
                    });
                    formState.desc = response.data.data.desc;
                    formState.utilities = JSON.parse(response.data.data.utilities);
                })
                .catch((error) => {
                    console.log(error);
                });
        ...
        const onFinish = values => {
            axios.put(`http://127.0.0.1:8000/api/homestays/${route.params.id}`,
                {
                    ...values,
